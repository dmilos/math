           ...
           class chuchc
            {
             /*
              ...
              Jen-Hui Chuang, Chih-Hui Ho, Ardian Umam, Hsin-Yi Chen, Jenq-Neng Hwang, Tai-AnChen
             ...
             public:
               ...
               /*!
                @param value - focus
                @param point - use multiple measure for principal line and make intersection
                @param princip - can be calculated from H, use ::math::linear::homography::principal().process(H).axis()
                @param H - plane to display( raster image ), no constraints
               */
               bool process( scalar_name & value, uv_type const& point, ABC2D_type const& princip, homography_type const& H )
                {
                 ...
                 ::math::linear::matrix::scale( Hnew, scalar_name(1) / Hnew[2][2] );

                 scalar_name cos_gamma = Hnew[1][1]/Hnew[0][0];
                 if( 1 < fabs( cos_gamma ) )
                  {
                   return false;
                  }

                 scalar_name sin_gamma = sqrt( scalar_name(1) - cos_gamma * cos_gamma );

                 auto s = Hnew[2][1]/sin_gamma;
