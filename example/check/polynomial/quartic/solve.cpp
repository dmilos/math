...
{
  std::array<double,4>   rF4,rD4, rS4;

  std::array<double,5> depressed/*, revert*/;
  ...
  if( errorF < eF )
   {
    errorF = eF;
    std::cout << "\033[31m" << "ferrari E: " << errorF << "; C: " << coefficients[0] << ", " << coefficients[1] << ", " << coefficients[2] << ", " << coefficients[3] << ", " << coefficients[4] << "; " << std::endl;
   }
  ...
  switch( countS )
   {
    case( 4 ): eCurrent = std::max( eCurrent, fabs( ::math::polynomial::evaluate( -rS4[3], monic ) ) );
    case( 3 ): eCurrent = std::max( eCurrent, fabs( ::math::polynomial::evaluate( -rS4[2], monic ) ) );
    case( 2 ): eCurrent = std::max( eCurrent, fabs( ::math::polynomial::evaluate( -rS4[1], monic ) ) );
    case( 1 ): eCurrent = std::max( eCurrent, fabs( ::math::polynomial::evaluate( -rS4[0], monic ) ) );
    case( 0 ): ;
   }
  errorS = std::max( errorS, eCurrent );
  if(eS < errorS )
   {
      std::cout << "\033[32m"  << "shmakov E: " << errorS << "; C: " << coefficients[0] << ", " << coefficients[1] << ", " << coefficients[2] << ", " << coefficients[3] << ", " << coefficients[4] << "; " << std::endl;
      errorS = errorS;
   }
  ...
  if( errorD < eD )
   {
    errorD = eD;
    std::cout << "\033[34m" << "descartes E: " << errorD << "; C: " << coefficients[0] << ", " << coefficients[1] << ", " << coefficients[2] << ", " << coefficients[3] << ", " << coefficients[4] << "; " << std::endl;
   }

 if( countF != countS )
  {
   //std::cout << "countF != countS: " << countF << " != " << countS << std::endl;
  }

 if( countF != countD )
  {
   //std::cout << "countF != countD: " << countF << " != " << countD << std::endl;
  }
 if( countS != countD )
  {
  // std::cout << "countF != countS: " << countD << " != " << countD << std::endl;
  }
...
int main( int argc, char *argv[] )
 {
   ...
   for( coeffcient[1] = -1; coeffcient[1] < +1; coeffcient[1] += 0.01 )
    for( coeffcient[2] = -1; coeffcient[2] < +1; coeffcient[2] += 0.01 )
     for( coeffcient[3] = -1; coeffcient[3] < +1; coeffcient[3] += 0.01 )
      for( coeffcient[4] = -1; coeffcient[4] < +1; coeffcient[4] += 0.01 )
       {
  ...
  std::cout << "shmakov: " << errorD << std::endl;
