...
#if USE_PNM
    { std::ofstream( std::string( "hilbert2D-")+ (iteration < 10?"0":"") +std::to_string( iteration )+"-Rotate-256.pgm" ) << PNM::save( image, width, height, PNM::P2 ); }
#endif
   }
...
#if USE_PNM
   { 
    { std::ofstream( std::string( "hilbertND-")+ (iteration < 10?"0":"") +std::to_string( iteration )+"-Rotate-256.pgm" ) << PNM::save( image, width, height, PNM::P2 ); }
   }
#endif
        }
